...
def conv_ae( batch_input_shape, latent_dim=2, intermediate_dim=128, nb_filters=32, nb_conv=3, epsilon_std = 0.01):
    ...
    # Sampling
    z = Dense(intermediate_dim, activation='relu')(f)
    z_mean = Dense(latent_dim, activation='relu')(z)
...
def run_training( data_h5, data_type, training_args, model_args, result_dir ):
    ...
    # Load model
    vae, encoder = conv_ae( batch_input_shape = batch_input_shape,
...
def get_model_name( data_type, training_args, model_args ):
    ...
    name = data_type + "_CNNAE"
    for k,v in arg_list:
        if name != data_type + "CNNAE":
            name += "_"
        name += k + str(v)
