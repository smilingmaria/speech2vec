...
class Seq2seqAutoencoder(object):
    def __init__(self, batch_input_shape, cells, hidden_dim, encode_dim, depth, dropout_keep_prob, **kwargs):
        """
        Arguments:
        ...
        namestring += '_encode_dim' + str(encode_dim)
    ...
    def build_encoder(self):
        ...
        if bidirectional:
            self.encoder_output, _ = bidirectional_encoder( self.en_cell, self.x, self.keep_prob )
        else:
            self.encoder_output = basic_encoder( self.en_cell, self.x, self.keep_prob)

    def build_code(self):
        self.code = fullyconnected2D( self.encode_dim, self.encoder_output, self.keep_prob, name='code' )

    def build_decoder(self):
        peek = self.model_options['peek']

        decoder_input = fullyconnected2D( self.hidden_dim, self.code, self.keep_prob, name = 'decoder' )

        self.x_rec = basic_decoder( self.batch_input_shape, self.de_cell, decoder_input,  self.keep_prob, peek = peek )
